...
export default function TotalGoldChart({ sessionsBySpot, sessionsByClass, sessionsByDate, viewTab }: Props) {
  let data: { name: string; value: number }[] = []

  if (viewTab === 'overtime') {
    data = sessionsByDate.map(s => ({ name: s.date.slice(5), value: s.totalGold }))
  } else if (viewTab === 'class') {
    data = sessionsByClass.map(s => ({ name: VOCATION_LABELS[s.vocation] ?? s.vocation, value: s.totalGold }))
  } else {
    data = sessionsBySpot.map(s => ({ name: s.spotName, value: s.totalGold }))
  }
  ...
  return (
    <div className="bg-[#1a1a1a] rounded-xl border border-[#2e2e2e] p-4 flex flex-col flex-1">
      <h3 className="text-sm font-semibold text-white mb-3 flex items-center gap-2">
        <span>🪙</span> Total Gold per Hunt
      </h3>

      {!hasData ? (
        <div className="flex flex-col items-center justify-center flex-1 py-10 text-gray-600">
