...
export default function SilverPerHourChart({ sessionsBySpot, sessionsByClass, sessionsByDate, viewTab }: Props) {
  const isOvertime = viewTab === 'overtime'
  let data: { name: string; value: number }[] = []

  if (isOvertime) {
    data = sessionsByDate.map(s => ({ name: s.date.slice(5), value: s.silverPerHour }))
  } else if (viewTab === 'class') {
    data = sessionsByClass.map(s => ({ name: VOCATION_LABELS[s.vocation] ?? s.vocation, value: s.silverPerHour }))
  } else {
    data = sessionsBySpot.map(s => ({ name: s.spotName, value: s.silverPerHour }))
  }
      ...
      {!hasData ? (
        ...
        <ResponsiveContainer width="100%" height={200}>
          <LineChart data={data} margin={{ top: 0, right: 8, left: 0, bottom: 0 }}>
            ...
            <Tooltip
              formatter={(val: number) => [formatSilver(val) + '/h', 'Silver/Hour']}
